...
class MyReservationModifyScreen extends Component {
    ...
    componentDidMount() {                           // 렌더링전에 DOM에서 houseId 파라미터 전달받아 두기
        if (this.props.route.params) {
            const { reservationId } = this.props.route.params;
    ...
    // axios를 활용한 api통신을 통해 서버에 예약 수정 요청을 보내는 함수
    async postReserationModifyInfoData() {
        try {
    ...
    async getReservationData() {                      // axios를 활용한 api통신을 통해 서버로부터 예약관련 데이터들을 불러오는 함수
        try {
            const token = await getToken();
            const { reservationId } = this.props.route.params;
            console.log("get 요청내의 reservationId: ",reservationId);
    ...
    return (
        ...
        <View style={styles.container}>
            <View style={styles.titleView}>
                ...
                <Text style={styles.reservationText}> 예약 정보 수정하기 </Text>
            </View>
            ...
            <TouchableOpacity style={styles.reservationBtn} onPress={() => this.postReserationModifyInfoData()}>
